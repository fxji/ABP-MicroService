...
{
    public class ImageAppService : FileAppService
    {
        ...
        public ImageAppService(IRepository<FileInfo, Guid> repository, FileManager fileManager) : base(repository, fileManager)
        {
        ...
        public async Task<FileInfoDto> UploadImg([Required] string name, [Required] IFormFile file)
        {
            ...
            string uploadsFolder = Path.Combine(Environment.CurrentDirectory, "wwwroot", "Images");
            ...
            //TODO：文件md5哈希校验
            var result = await _fileManager.Create(name, uniqueFileName, fileExtension, "", size, filePath, "/images/" + uniqueFileName, FileType.Image);
