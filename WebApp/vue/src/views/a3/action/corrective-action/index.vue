...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios.gets('api/AAA/correctiveAction', this.listQuery).then(response => {
    ...
    fetchData(id) {
      this.$axios.gets('api/AAA/correctiveAction/' + id).then(response => {
        this.form = response;
      });
    },
    handleFilter() {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否删除' + alert + '?', '提示', {
        ...
        this.$axios.posts('api/AAA/correctiveAction/delete', params).then(response => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
    ...
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.isEdit) {
            this.$axios.posts('api/AAA/correctiveAction/data-post', this.form).then(response => {
